...
from lcapy import Circuit, s, f, j, omega, expr
import numpy as np
...
cir = Circuit("""
    ...
    Vj1 0_11 0_111 noise ; down
    Vj2 0_21 0_211 noise ; down
...
cirs = cir.subs(simple_dict)

cirs_eval = cirs.subs(an_dict)

response = cirs_eval[1].V.n

En = expr('(e0**2 +(ea/f**0.5)**2 + (eb/f)**2)**0.5 ')
...
f_array = 10**np.linspace(-1, 5, 1000)
En_array = En_eval.evaluate(f_array)

plt.figure()
plt.loglog(f_array, En_array)
plt.grid()
