...
for source, noise_expr in noise_expr_dict.items():
    ...
    ax.loglog(freq_array, noise_array, color=col, ls=ls)
...
leg1 = ax0.legend(
    handles=[
        ...
        plt.Line2D([], [], lw=1, color='slateblue', ls='--'),
        plt.Line2D([], [], lw=1, color='red', ls='-'),
        plt.Line2D([], [], lw=1, color='red', ls='--'),
        plt.Line2D([], [], lw=1, color='red', ls='dotted'),
        '',
        plt.Line2D([], [], lw=1, color='forestgreen', ls='-'),
        plt.Line2D([], [], lw=1, color='forestgreen', ls='--'),
        plt.Line2D([], [], lw=1, color='orange', ls='-'),
        plt.Line2D([], [], lw=1, color='orange', ls='--'),
    ...
    loc='center left', bbox_to_anchor=(1.25, 0.75), frameon=False, ncol=2
)
...
ax0.legend(
    handles=[
        ...
        plt.Line2D([], [], lw=1.5, ls='dotted', color='k'),
        plt.Line2D([], [], lw=1.5, ls='-.', color='k'),
    ],
    labels=[
        '',
        '$I_s^{{bulk}}$\n$\sigma_{{bulk}}^B = {:.1f}$ eV'.format(res_dict['Ibulk']*1000),
        '$I_s^{{veto}}$\n$\sigma_{{veto}}^B = {:.1f}$ eV'.format(res_dict['Iveto']*1000),
        '$I_s^{{equator}}$\n$\sigma_{{equator}}^B = {:.0f}$ eV'.format(res_dict['Iequator']*1000),
    ],
    handler_map={str: LegendTitle()},
    loc='center left', bbox_to_anchor=(1.25, 0.25), frameon=False
)
...
### Saving
fig.savefig('fid38_noise_propagation.pdf')
